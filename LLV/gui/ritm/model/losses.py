    ...
    def forward(self, pred, label):
        ...
        with torch.no_grad():
            ignore_area = torch.sum(label == self._ignore_label,
                                    dim=tuple(range(1, label.dim()))).cpu().numpy()
        ...
        if self._size_average:
            bsum = torch.sum(sample_weight,
                             dim=misc.get_dims_with_exclusion(sample_weight.dim(),
                                                              self._batch_axis))
            loss = torch.sum(loss, dim=misc.get_dims_with_exclusion(
                loss.dim(), self._batch_axis)) / (bsum + self._eps)
        else:
...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, label, sample_weight=None):
        ...
        if self._size_average:
            ...
            loss = torch.sum(loss, dim=misc.get_dims_with_exclusion(
                loss.dim(), self._batch_axis)) / (tsum + self._eps)
        else:
            loss = torch.sum(loss, dim=misc.get_dims_with_exclusion(loss.dim(), self._batch_axis))
...
class SigmoidBinaryCrossEntropyLoss(nn.Module):
    ...
    def forward(self, pred, label):
        ...
        if not self._from_sigmoid:
            ...
            eps = 1e-12
            loss = -(torch.log(pred + eps) * label + torch.log(1. - pred + eps) * (1. - label))

...
def load_event_voxel(event_root, video, frames_idx):
    """
    Load a sequence of event voxel .npy files for one video.

    Args:
        event_root: root directory where event folders are stored
        video: str, video folder name (e.g., 'bear_01')
        frames_idx: list of frame indices to load

    Returns:
        Tensor of shape [T, 5, H, W]
    """
    event_sequence = []
    for frame_id in frames_idx:
        npy_path = os.path.join(event_root, video, f"{frame_id+1:05d}.npy")
        if not os.path.exists(npy_path):
            print(f"[Warning] Missing event voxel: {npy_path}, skipping.")
            continue
        voxel = np.load(npy_path)  # shape: [5, H, W]
        voxel_tensor = torch.from_numpy(voxel).float()
        event_sequence.append(voxel_tensor)

    # sequence 부족 체크
    if len(event_sequence) != len(frames_idx):
        raise ValueError(f"[Error] Incomplete event voxel sequence: expected {len(frames_idx)}, got {len(event_sequence)}")

    return torch.stack(event_sequence, dim=0)  # [T, 5, H, W]
...
class VOSMergeTrainDataset(Dataset):
    ...
    def __init__(self, data_configs, seq_length=3, max_num_obj=3, size=480, merge_probability=0.0):
        ...
        for dataset, config in data_configs.items():
            ...
            for vid in vid_list:
                ...
                self.video_frames.extend([(dataset, vid, i) for i in range(1, len(frames))] * multiplier)
                #self.video_frames.extend([(dataset, vid, i) for i, _ in enumerate(frames)] * multiplier) # 여기도 idx 0 생성 가능
    ...
    def _get_sample(self, idx=None):
        ...
        if idx is None:
            idx = np.random.randint(1, len(self.video_frames))
            ...
            for seed_trial in range(self.max_seed_trials):
                ...
                if not seed_ok:
                    if seed_trial == self.max_seed_trials - 1:
                        ...
                        frames_idx = [np.random.randint(1, length)]
                        continue
                """
                ...
                for i, f_idx in enumerate(frames_idx):
                    ...
                    png_name = frames[f_idx].split(':')[0]  # ':' 기준 앞부분만 사용 
                    jpg_name = png_name
                ...
                event_root = config['event_root']  # 이 항목은 setup_training_data에서 config에 포함돼야 함
                try:
                    event_voxel = load_event_voxel(event_root, video, frames_idx) # [T, 5, H, W]
                except ValueError as e:
                    print(e)
                    return self._get_sample()  # 재시도
                #event_voxel = load_event_voxel(event_root, video, frames_idx)  # [T, 5, H, W]
                # event 이미지도 RGB 데이터 같이 480x480으로 resize
                event_voxel = torch.nn.functional.interpolate(
                    event_voxel, size=(480, 480), mode='bilinear', align_corners=False
                )
                return info, images, masks, event_voxel
    ...
    def __getitem__(self, idx):
        ...
        info, images, masks, event_voxel  = self._get_sample(idx)
        ...
        if num_labels < self.max_num_obj and np.random.rand() < self.merge_probability:
            _, images2, masks2, _ = self._get_sample()

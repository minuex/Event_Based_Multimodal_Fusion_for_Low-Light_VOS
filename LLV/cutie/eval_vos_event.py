...
def eval_vos(cfg: DictConfig):
    ...
    #이벤트 불러오기 추가 ================================================================
    event_dir = data_cfg.get('event_directory')
    #=================================================================================
    ...
    meta_dataset = VOSTestDataset(image_dir,
                                  ...
                                  size_dir=size_dir,
                                  subset=subset,
                                  event_dir=event_dir)  # event = keyword 인자로 정확히 명시
    use_amp = cfg.amp
        ...
        try:
            ...
            for ti, data in enumerate(loader):
                with torch.cuda.amp.autocast(enabled=use_amp):
                    ...
                    #log.info(f"[eval_vos_event] event tensor shape: {event.shape}")
                    mask = data.get('mask')
                    if mask is not None:
                        mask = mask.cuda()
                    ...
                    if not first_mask_loaded:
                        if mask is not None:
                            ...
                            continue
                    # inference_core.py 의 step 불러와서 추론
                    #log.info(f"[before step] event shape: {event.shape}") [1, 5, H, W]
                    prob = processor.step(image, event, mask, valid_labels, end=(ti == vid_length - 1))

...
class VideoReader(Dataset):
    ...
    def __init__(
        ...
        image_dir: str,
        mask_dir: str,
        event_dir: Optional[str] = None, # event 추가
        *,
        size: int = -1,
        ...
        self.event_dir = event_dir # event 추가
    ...
    def __getitem__(self, idx):
        ...
        # event 로드
        if self.event_dir :
            event_idx = int(frame[:-4]) # + 1  # 예: frame '00000.jpg' → event '00001.npy'
            # 이벤트 경로 설정
            event_path = path.join(self.event_dir, f"{event_idx:05d}.npy")
            event = np.load(event_path)  # [5, H, W]
            event = torch.from_numpy(event).float()  # numpy → Tensor (float32)
            event = event.unsqueeze(0)  # [1, 5, H, W] 
            data['event'] = event # Tensor [1, 5, H, W]

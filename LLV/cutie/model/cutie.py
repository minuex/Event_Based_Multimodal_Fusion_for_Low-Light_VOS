...
class CUTIE(nn.Module):
    def __init__(self, cfg: DictConfig, *, single_object=False):
        ...
        self.pix_feat_proj = nn.Conv2d(self.ms_dims[0], self.pixel_dim, kernel_size=1) # 256으로
        # 이벤트 인코더 추가
        self.event_encoder = EventEncoder(model_cfg)
        self.event_feat_proj = nn.Conv2d(self.ms_dims[0], self.pixel_dim, kernel_size=1) # 256 으로
        # 이벤트 데이터가 생김
        # ACMF 추가
    ...
    # image 인코더 + event 인코더 + acmf 합치기
    def encode_image_with_event(self, image: torch.Tensor, event: torch.Tensor):
        image = (image - self.pixel_mean) / self.pixel_std
        # image 인코딩 + proj
        ms_image_feat = self.pixel_encoder(image)
        img_feat = self.pix_feat_proj(ms_image_feat[0])
        # event 인코딩 + proj
        evt_encode = self.event_encoder(event)
        evt_feat = self.event_feat_proj(evt_encode)
        # ACMF
        fused = self.acmf(img_feat, evt_feat)
        return ms_image_feat, fused

    # event encoder 함수 추가
    def encode_event(self, event: torch.Tensor):
        return self.event_encoder(event)
    
     # ACMFMoudle 함수 추가
    def forward_acmf(self, rgb_feat: torch.Tensor, event_feat: torch.Tensor):
        return self.acmf(rgb_feat, event_feat)
    ...
    def forward(self, rgb: torch.Tensor, event: torch.Tensor):
        ...
        """
        #이미지 encoder
        ms_feat, pix_feat = self.encode_image(rgb)
        #event encoder
        evt_feat = self.event_encoder(event)
        evt_feat = self.event_feat_proj(evt_feat)
        #이벤트 특징 데이터 생성 완료
        #ACMF로 두 특징 합침
        fused_feat = self.acmf(pix_feat, evt_feat)

        return ms_feat, fused_feat

    def train(self, mode=True):
        super().train(mode)  # ✅ PyTorch 기본 train 호출
        return self

    def eval(self):
        return self.train(False)  # ✅ eval은 train(False)
    '''
    def train(self, mode=True):
        self.train(mode)
        self.event_encoder.train(False)
        self.acmf.train(False)
        self.training = mode
        return self

    # Cutie eval_vos_event.py의 30줄 cutie = CutieEvalWrapper(cfg).cuda().eval()
    def eval(self):
